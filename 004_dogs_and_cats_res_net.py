...
# Шаг 2: Подготовка данных (для разнообразия не будем использовать class CustomDataset(Dataset))
...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        # Шаг обновления learning rate
        scheduler.step()

        # Печатаем текущий learning rate
        current_lr = optimizer.param_groups[0]['lr']        

        print(f'Epoch {epoch + 1}/{num_epochs}, Loss: {epoch_loss:.4f}, Accuracy: {accuracy:.4f}, Learning Rate {current_lr}')
